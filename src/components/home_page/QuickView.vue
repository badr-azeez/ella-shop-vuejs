<template>
  <div class="text-center pa-4">
    <v-dialog v-model="dialog" max-width="1000">
      <v-card>
        <v-row class="px-4">
          <v-col v-if="loading" class="my-10" cols="12" md="5">
            <v-skeleton-loader
              type="image,image,image,image"
            ></v-skeleton-loader>
          </v-col>

          <v-col v-if="!loading" class="my-10 px-4" cols="12" md="5">
            <img
            ...
            <v-tabs v-model="openItem" fixed-tabs height="100">
              <v-tab
                v-for="(img, i) in productItem.images"
                :key="i"
                :value="img"
              >
          ...
          <v-col v-if="loading" class="my-10 px-4" cols="12" md="7">
            <v-skeleton-loader
          ...
          <v-col v-if="!loading" class="my-10 px-4" cols="12" md="7">
            <div class="title">
              <p class="text-h5">
                ({{ productItem.title }}) Sample -
                {{ productItem.category }} For Sale
              </p>
            </div>
            ...
            <div class="price mt-4 text-h6">
              ...
              <span class="font-weight-black">
                {{
                  (
                    (productItem.price *
                      (100 - productItem.discountPercentage)) /
                    100
                  ).toFixed(2)
                }}
              </span>
            </div>
            <div class="quantity mt-4 w-sm-25">
        ...
        <template v-slot:actions>
          <v-btn
            ...
            icon="mdi-close"
            @click="dialog = false"
            style="position: absolute; top: 1px; right: 1px; font-size: 18px"
          ></v-btn>
        </template>
      </v-card>
    </v-dialog>
  ...
  methods: {
    ...
    addCartItem(item) {
      ...
      setTimeout(() => {
        ...
        this.Emitter.emit("openSnackbar", `${item.title} Was Added`);
      }, 500);
    },
